...
usedIPS=$(arp-scan -I eth0 ${ipAddr[0]}.${ipAddr[1]}.${ipAddr[2]}.0/24 |  grep -E -o "(1[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))
for i in "${usedIPS[@]}"; do
    echo "$i"
done
